...
const isFolderOwner = async (req, res, next) => {
  ...
  try {
    const folder = await browserQueries.getFolder(folderId);
    if (!folder) {
      const err = new Error("Not Found");
      err.status = 404;
      return next(err);
...
const isSharedFile = async (req, res, next) => {
  try {
    ...
    if (await fileQueries.isSharedFile(fileId, sharedFolderId)) {
      return next();
    } else {
      const err = new Error("Not Found");
      err.status = 404;
...
export { isAuth, isFolderOwner, isSharedFolder, isSharedDescendant, isSharedFile, isFileOwner };
